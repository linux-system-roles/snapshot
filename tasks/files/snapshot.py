...
# # TODO should move these functions to another file - but need to
# figure out how Ansible deals with the imports
...
def verify_created(vg_name, lv_name, prefix, suffix):
    ...
    for list_item in report:
        ...
        for lvs in list_item["lv"]:
            ...
            if lv_exists:
                ...
                if not is_snapshot:
                    return SnapshotStatus.ERROR_VERIFY_NOTSNAPSHOT, "LV exits, but it is not a snapshot"
            else:
                return SnapshotStatus.ERROR_VERIFY_NOTSNAPSHOT, "snapshot not found for source LV"
...
def verify_snapshots_removed(vg_name, lv_name, prefix, suffix):
    ...
    if vg_name and lv_name:
        ...
        if rc != SnapshotStatus.SNAPSHOT_OK:
            return SnapshotStatus.ERROR_VERIFY_REMOVE_FAILED, "clean verify: command failed for LV lvm_is_snapshot() failed to get status on source"
        if is_snapshot:
            return SnapshotStatus.ERROR_VERIFY_REMOVE_SOURCE_SNAPSHOT, "clean verify: source is a snapshot:" + vg_name + "/" + lv_name
    ...
    for list_item in report:
        ...
        for lvs in list_item["lv"]:
            ...
            rc, is_snapshot = lvm_is_snapshot(verify_vg_name, lvs["lv_name"])
            if rc != SnapshotStatus.SNAPSHOT_OK:
                return SnapshotStatus.ERROR_VERIFY_REMOVE_FAILED, "clean verify: command failed for LV lvm_is_snapshot() failed to get status"
            ...
            if lv_exists:
                return SnapshotStatus.ERROR_VERIFY_REMOVE_FAILED, "clean verify: volume exists that matches the pattern: " + verify_vg_name + "/" + snapshot_name
...
def error_print(*error_args, **kwargs):
    print(*error_args, file=sys.stderr, **kwargs)
